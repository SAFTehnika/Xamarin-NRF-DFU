...
namespace Plugin.XamarinNordicDFU
{
    ...
    /// nRF5_SDK => nrf_dfu_ext_error_code_t
    /// </summary>
    public enum DFUOperationExtendedErrorCode : byte
    {
    ...
    /// nRF5_SDK => nrf_dfu_result_t
    /// </summary>
    public enum DFUOperationResultCode : byte
    {
    ...
    partial class DFU
    {
        ...
        public async Task Start(IDevice device, Stream FirmwarePacket, Stream InitPacket, string dfuAdvertName = "DfuTarg")
        {
            ...
            try
            {
                ...
                newDevice = await ButtonlessDFUWithoutBondsToSecureDFU(device, dfuAdvertName);
                ...
                DFUEvents.OnLogMessage?.Invoke("DFU process completed successfully.");
            }
            catch (Exception ex)
            {
                DFUEvents.OnError?.Invoke(ex.Message);
        ...
        public async Task<bool> IsReadyForEnteringDfuMode(IDevice device)
        {
            if (device != null && device.IsConnected())
            {
                IGattCharacteristic buttonlessCharacteristic = await device.GetKnownCharacteristics(DfuService, DfuButtonlessCharacteristicWithoutBonds).Timeout(OperationTimeout);
                return buttonlessCharacteristic != null;
            }
            return false;
        }
    }
